...
public class StartNotificationReminder extends BroadcastReceiver {
  ...
  public void onReceive(Context context, Intent intent) {
    ...
    if (Intent.ACTION_BOOT_COMPLETED.equals(intent.getAction()) && nextReminder == 0) {
      ...
      // reminder date is far in the future, schedule an alarm
      final AlarmManager m = (AlarmManager) context.getSystemService(Context.ALARM_SERVICE);
      if (m != null)
        m.setRepeating(AlarmManager.RTC_WAKEUP, nextReminder, Constants.ONE_DAY_MS, getBroadcastIntent(context));
    } else {
      final SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
      // only show a notification if inbound lightning payments are enabled
