...
public class OpenChannelCapacityFragment extends Fragment {
  ...
  private Long extractFundingFees(final String feesString) {
    ...
    try {
      ...
      if (feesSatPerByte != getApp().estimateSlowFees() && feesSatPerByte != getApp().estimateMediumFees() && feesSatPerByte != getApp().estimateFastFees()) {
      ...
      if (feesSatPerByte <= getApp().estimateSlowFees() / 2) {
        mBinding.setFeesWarning(getString(R.string.payment_fees_verylow));
      } else if (feesSatPerByte >= getApp().estimateFastFees() * 2) {
