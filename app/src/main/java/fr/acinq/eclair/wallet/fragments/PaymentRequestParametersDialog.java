  ...
  private MilliSatoshi extractAmount(final String amountString) {
    try {
      ...
      if (Strings.isNullOrEmpty(amountString)) {
        ...
        final MilliSatoshi amount = new MilliSatoshi(CoinUtils.convertStringAmountToMsat(amountString, prefUnit.code()).toLong());
        package$.MODULE$.satoshi2btc(amount.truncateToSatoshi());
        ...
        if (amount.toLong() > maxReceivableAmount.toLong()) {
